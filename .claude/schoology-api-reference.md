...
| POST | `/v1/multiget` (**BULK profiles, verified 2026-06-07**) | 207 | **The real batch mechanism.** Body `{ request: ["/v1/users/{uid1}", "/v1/users/{uid2}", …] }` (paths MUST carry the `/v1` prefix; the `{requests:[/users/…]}` form returns empty). Returns `{ response: [{ location, response_code, body:{…full /users/{uid}…} }] }` — each bundled `body` is the COMPLETE user object incl. `primary_email` **and** `parents.parent[]`, in ONE HTTP call. Verified: 4 uids → 4 bundled 200s, each with email + parents. **This replaces the per-student `getUserProfile` walk (`sync.js:555`) — bundle students (Schoology caps multiget at ~50/request, so chunk) into ceil(N/50) calls instead of N.** (`grad_year` still absent for students — that's PowerSchool data per the `/users/{uid}` note above.) |
...
| `POST /course/{id}/district_mastery/api/material-observations/search` (**BATCHED, #55-sibling, verified 2026-06-07**) | Body `{ building_id, section_id, objective_ids: "uuid1,uuid2,…" }` (CSRF-token/key headers, same as `alignments/search`). **Returns ALL observations for ALL listed topics in ONE call** — verified on ACSS `7899896098`: the POST returned 212 observations spanning all 8 objectives, identical to the sum of 8 per-topic GETs. Each observation carries `objective_id`, so regroup by it to rebuild `observationsByTopic`. ⚠️ The GET form does NOT batch (`objective_ids` plural → 500; `objective_id={csv}` → 400). **This collapses the per-topic loop (N calls → 1) — the mastery analog of the #55 bulk submissions endpoint.** |
