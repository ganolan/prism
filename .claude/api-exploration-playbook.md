...
- **PowerSchool `/ws/pt/v1/...` (PowerTeacher v1) + `/ws/seatingchart/...`** — partly done 2026-05-30: `getattendance_integration?sectionid={dcid}&date={day}` shape-probed & documented (richer than legacy `section_attendance`; incl. code catalog). **Still open:** `getattendanceformultisection` (same params, not separately probed), all `/ws/seatingchart/...` + `/ws/pt/v1/seating_chart/...`, `/ws/pt/v1/student/...` (PII), `/ws/preferences/core/(user/)pref/{name}`, `/ws/session/*`. `saveattendance(s)` are writes — out of scope.
- ~~Schoology React bundles on `asset-cdn.schoology.com` (grep for endpoint literals)~~ **DONE 2026-05-30 — negative.** Grepped 8 react-common/common bundles; ~47 route literals all 404 (generic SDK templates, not live HKIS routes). See schoology-api-reference.md "React-bundle literal-grep — negative finding." Driving real pages is the reliable method instead.
- **Schoology internal gradebook/rubric web routes** (`/grades/*`, `/gradebook/*`) — **partly done 2026-05-30**: driving the gradebook found `/gradebook/{sectionId}/visualization_data` (per-student per-category grade breakdown) + `/enrollments/{id}/limited_gradebook_session`. The grade **grid is server-rendered HTML** (no isolated JSON). Rubric-criteria **not** found here (the `grading_rubrics` 403 still has no internal-web workaround; `district_mastery/api` remains the source). Still open: the mastery grid's own XHRs (`/grades/mastery/{id}`) weren't isolated; the React `course-dashboard` app's data calls not driven.
