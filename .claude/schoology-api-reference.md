...
| GET | `/v1/sections/{id}/submissions/{aid}` (**no uid — BULK, #55, verified 2026-06-07**) | 200 | **All students' revisions for an assignment in ONE call** — `{ revision: [{ revision_id, uid, created, num_items, late, draft }], total, links }`, paginated (`links.next`; ~20/page). Same per-revision fields as the per-student form; cross-checked identical (uid 11862763 on native-dropbox `8207125849`/sec `7899907720`: late/draft/created match). **This is the bulk replacement for the per-(assignment×student) submission walk for NATIVE dropbox** — group revisions by `uid`, compute per-student late/draft/latest-non-draft `created` (for #49 resubmit timing). ⚠️ For **lti_submission** work the public revisions API is blind (auto-provisioned empty drafts only) — lti uses the `/iapi2/.../{submitted,in-progress}-documents/` endpoints instead (#62), which **403 for native dropbox**. So: lti → documents endpoints; native dropbox → this bulk endpoint. Together they retire the per-cell walk. |
