...
- **PowerSchool `/ws/pt/v1/...` (PowerTeacher v1) + `/ws/seatingchart/...`** — `getattendance_integration` is LIVE but the **correct section param is unknown**: all guessed variants 409 `Invalid sectionId: 0` (with `Accept: application/json`; 500 without). Must **capture the grid's real request** to get the true param + shape — reconstruction failed. Legacy `/ws/attendance/section_attendance` stays the only confirmed attendance read. *(2026-06-08, #106: captured the live grid render — `getattendance_integration` did **not** fire on the default render, so its signature is still unresolved; it must come from a different view/interaction. But the **block-number** goal that motivated it is solved without it — the displayed "Block N" is `section_info.bellScheduleItems[].period.name`; see powerschool-api-reference.md "Block number".)* **Still open:** the real `getattendance_integration` call, `getattendanceformultisection`, all `/ws/seatingchart/...` + `/ws/pt/v1/seating_chart/...`, `/ws/pt/v1/student/...` (PII), `/ws/preferences/core/(user/)pref/{name}`, `/ws/session/*`. `saveattendance(s)` = writes, out of scope.
