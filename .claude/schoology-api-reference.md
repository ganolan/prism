...
- **Enrollments**: `GET /sections/{id}/enrollments` returns all members. Filter by `admin !== 1` to get students only — **and by `status` to get *currently enrolled* students (#128).** ⚠️ **A dropped student is NOT removed from this response.** The row keeps being returned with `status: "5"` instead of `"1"`; nothing disappears. Filtering on `admin` alone (which Prism did until 2026-08-13) re-adds dropped students on every sync. **Observed values — `"1"` = active, `"5"` = dropped; the rest of the enum is unverified.** Verified 2026-08-13 by sweeping all 9 active HKIS sections (`scripts/probe-enrolment-status-sweep.js`): 117 student rows → `"1"` × 115, `"5"` × 2, and the two `"5"` rows were exactly the two students known to have dropped (Anant Sachdeva, AP CSP `8458134140`; Ho Kan Lai, AIML `8458134359`). Full object for a dropped row: `{id, uid, school_uid, name_title, name_title_show, name_first, name_first_preferred, use_preferred_first_name, name_middle, name_middle_show, name_last, name_display, admin: 0, status: "5", picture_url, enrollment_source, links{self}}`. Prism treats only `"1"` as active (`server/lib/enrolmentStatus.js`) and soft-flags the rest via `enrolments.dropped_at`.
...
| GET | `/v1/users?uids=...` | 200 | NOT a multi-get — returns a paginated list of ALL users (ignores the `uids` param). `total: 1914`. Useless for batched profile lookup. |
| POST | `/v1/multiget` (**BULK profiles, SHIPPED #105 2026-06-07**) | 207 | **The real batch mechanism.** Body `{ request: ["/v1/users/{uid1}", "/v1/users/{uid2}", …] }` (paths MUST carry the `/v1` prefix; the `{requests:[/users/…]}` form returns empty). Returns `{ response: [{ location, response_code, body:{…full /users/{uid}…} }] }` — each bundled `body` is the COMPLETE user object incl. `primary_email` **and** `parents.parent[]`, in ONE HTTP call. **SHIPPED (#105):** `getUserProfilesBatch` (`schoology.js`, `apiPost` + chunk ≤50) + `enrichStudentProfiles` now use this instead of the per-student walk. **Live parity verified across all 211 students** (email + preferred-name + guardian set identical to per-student `GET /users/{uid}`; 0 mismatches — `scripts/parity-multiget-profiles.js`). Per-entry non-200 `response_code` is skipped; a failed chunk is best-effort (those students preserved). (`grad_year` still absent for students — PowerSchool data.) |
...
| GET | `/v1/sections/{id}/enrollments` | 200 | All enrolled users, **including students who have dropped** (see #128 note above). Fields: `id` (enrollment_id), `uid`, `school_uid`, `name_first`, `name_first_preferred`, `name_last`, `name_display`, `admin` (1=teacher/TA, 0=student), `status` (**`"1"` = active, `"5"` = dropped**; observed 2026-08-13, rest of enum unverified), `picture_url`, `enrollment_source`. Paginated. |
...
| Method | Endpoint | Status | Notes |
|--------|----------|--------|-------|
| GET | `/v1/sections/{id}/submissions/{aid}/{uid}` | 200 | Submission revisions for one student. Returns `{ revision: [{ revision_id, uid, created, num_items, late, draft }] }`. `late: 0/1`, `draft: 0/1`. |
| GET | `/v1/sections/{id}/submissions/{aid}` (**no uid — BULK, #55, SHIPPED 2026-06-07**) | 200 | **Every student's LATEST revision for an assignment in ONE call** — `{ revision: [{ revision_id, uid, created, num_items, late, draft }], total, links }`, paginated (`links.next`; ~20/page). ⚠️ **CORRECTION (2026-06-07 parity check):** the bulk form returns only the **latest revision per student**, **NOT** the full revision history — the earlier "All students' revisions" claim was wrong (it only cross-checked one single-revision student, uid 11862763, which couldn't reveal the truncation). Re-verified across MAD (`8230636099`/`8347039321`/`8376973073`) and AP CSP CPT projects (`7957396557`/`8030139112`/`8112119711`/`8202978174`/`8275878296`): per-student returned `[1,2,3…]` where bulk returned only the latest (up to 5/8 students truncated on MAD). **This is still sync-equivalent for NATIVE dropbox** and is what `getAssignmentSubmissions` + `syncSectionData` now use: the sync only needs the latest revision's late/draft and the newest **non-draft** `created`, and a genuine resubmit (newer non-draft) IS the bulk's latest — so `summarizeRevisions` derives the identical per-student summary (**0 summary-mismatches** across all probed assignments, incl. the heavy-resubmit ones; the only divergent case, "latest-by-id is a draft over an earlier non-draft", would surface as a summary mismatch and was never observed). Probe: `scripts/parity-bulk-submissions.js`. ⚠️ For **lti_submission** work the public revisions API is blind (auto-provisioned empty drafts only) — lti uses the `/iapi2/.../{submitted,in-progress}-documents/` endpoints instead (#62), which **403 for native dropbox**. So: lti → documents endpoints; native dropbox → this bulk endpoint. Together they retire the per-cell walk. |
...
**RESOLVED — the FULL 3-way state (Not Started / In Progress / Submitted), per assignment (2026-06-07 probe, #62).** GHD's `submission` key only answers *submitted vs not*; it cannot separate "opened, working" from "never opened" (a bare GHD cell is both). The Schoology grader's own **"In Progress" / "Submitted" tabs** (assignment React bundle `react-common/assignment-*.js`, found by literal-grep per the playbook) are fed by two **per-assignment** `/iapi2/` endpoints (browser-session auth, same session as GHD/mastery):
- `GET /iapi2/assignments/{assignmentId}/submitted-documents/` → students who **submitted**.
- `GET /iapi2/assignments/{assignmentId}/in-progress-documents/` → students **not yet submitted**.
Both return a flat array; each entry: `{ id (=schoology uid), enrollmentId, firstName, middleName, lastName, avatarUrl, submissionTiming, submissionStatus, exception, grade, revisionCreated, submissionDate }`. **The discriminator is `revisionCreated` (a real boolean):** in the in-progress list, `true` = the student created their own copy / opened it (**In Progress**); **`false` = never opened (**Not Started**)**. A student in *neither* list would also be not-started, but in practice every assigned student appears in in-progress until they submit. So per (student, assignment): `submitted-documents` ⇒ Submitted; `in-progress-documents` + `revisionCreated:true` ⇒ In Progress; `in-progress-documents` + `revisionCreated:false` ⇒ Not Started; `grade` present ⇒ Graded. **Verified** on Robotics section `7899907720`: Notebook 4 (`8348763574`) = 1 submitted + 18 in-progress, of which exactly 1 (`revisionCreated:false`, uid 132465441) was the teacher-confirmed never-opened student; the public revisions API could NOT find her (it returns 0 revisions for *both* her and the submitted student, whose OneDrive revision it hides). Notebook 3 (`8348763571`) = 17 submitted + 2 in-progress (both `true`) + 0 not-started. `submitted-documents` count == GHD `submission`-present count in both (1/1, 17/17). **`submissionDate` + `submissionTiming` — DECODED (2026-06-15, #125).** `submissionDate` is a human display string, present on **submitted** entries (empty `""` for in-progress/not-started). Verified format across 36 submitted entries (Notebook 4 `8348763574`, Notebook 3 `8348763571`): `"<Weekday>, <Month> <D>, <YYYY> at <h>:<mm> <am|pm>"` — e.g. `"Tuesday, June 9, 2026 at 3:27 pm"`. **Day and hour are NOT zero-padded; minute IS 2-digit; am/pm lowercase; no seconds; no timezone** (it's the HKIS Schoology account's display tz, i.e. HKT). **`submissionTiming` is a 3-value enum `{0,1,2}`** (not the previously-guessed 0/1): **`0` = no submission** (in-progress / not-started — `submissionDate` empty), **`1` = on-time** (submitted at/before the assignment's due datetime), **`2` = late** (submitted after due). Confirmed against due dates: Notebook 4 (due 2026-06-09 15:30) — uid 55454030 submitted June 9 **3:27 pm** (3 min before due) ⇒ `timing=1`, every June 10–15 submission ⇒ `timing=2`; Notebook 3 (due 2026-05-18 15:30) — earliest submission 5:12 pm (1h42m late) and all others ⇒ all `timing=2` (nobody on time). So `late = (submissionTiming === 2)`. **Prism (#125) parses `submissionDate` → epoch (server-local tz — Prism is local-first on an HKT host, so local = Schoology's display tz) into `grades.submitted_at`/`latest_revision_at`, and maps `submissionTiming` → `grades.late`, making LTI submission timing consistent with native dropbox.** Using `submissionTiming` (Schoology's own determination) for `late` is preferred over re-deriving from the parsed epoch — it sidesteps the timezone ambiguity entirely. **Cost note:** these are **per-assignment list reads** (2 calls cover the whole roster), strictly cheaper than the current O(students×assignments) public per-cell submission walk — relevant to #55. **Not yet scoped:** behaviour on archived/inactive sections (GHD is blind there — likely same), pagination on large rosters (HKIS sections ~20, returned inline), and cross-grading-period coverage (per-assignment URL, so likely fine — unlike GHD which is current-period only). Probe scripts: `scripts/probe-submission-documents.js`, `scripts/probe-lti-submission-states.js`, `scripts/probe-revision-detail.js`.

**Failure mode + Prism handling (#76 follow-up, 2026-06-14).** Both reads happen in one `fetchAssignmentSubmissionState` call per assignment via the shared browser session; if **both** come back null it returns null and the assignment yields **no** state for **any** student (all-or-nothing — there is no per-cell LTI fetch, so a single cell can't fail while a sibling on the same assignment succeeds). The call is hiccup-prone (a fresh page nav + two sequential XHRs; the in-progress read "can silently drop"), and unlike native dropbox it had **no retry** — the root cause of a same-sync assignment blanking while its sibling captured fine. Prism now (a) **retries the fetch once** (`retryAsync`, ~750 ms backoff) inline while the session is open, and (b) **persists the per-assignment outcome** in `assignments.lti_fetch_status` (`'ok'` | `'failed'`; `NULL` = non-lti / never-attempted / windowed-out — left untouched so a good full sync self-clears a stale `'failed'`). The gradebook shows a cell-level amber "submission status unavailable — re-sync" marker only on `'failed'` assignments; a `null` (unknown-but-not-failed) cell renders blank, never "Ungraded". A student genuinely absent from **both** lists on an `'ok'` assignment stays blank — it is not a fetch failure and re-sync may not change it.
...
| `GET /course/{id}/district_mastery/api/material-observations/search?building_id=...&objective_id=...&section_id=...` | Per-student per-assignment raw scores for a **single** measurement topic. `masterySync.js` currently loops this **once per topic** — the O(topics)/course cost (the ~20–40s/course mastery floor). |
| `POST /course/{id}/district_mastery/api/material-observations/search` (**BATCHED, SHIPPED #104 2026-06-07**) | Body `{ building_id, section_id, objective_ids: "uuid1,uuid2,…" }` (CSRF-token/key headers, same as `alignments/search`). **Returns ALL observations for ALL listed topics in ONE call** — each observation carries `objective_id`, regroup by it to rebuild `observationsByTopic`. ⚠️ The GET form does NOT batch (`objective_ids` plural → 500; `objective_id={csv}` → 400). ⚠️ **SHAPE DIFFERS from the per-topic GET** (caught by the #104 parity check): the POST nests the material id at `gradeable_material.material.id` (the GET uses `gradeable_material.material_id`), and `points` is a **string** (`"100"`) not a number; "no score" is `points: null` (POST) vs the field absent (GET). `masterySync` normalizes each POST row to the GET shape via `normalizeObservation` (`server/lib/masteryObservations.js`) before the persist. **SHIPPED (#104):** `syncCourseMastery` replaced the per-topic GET loop (N calls → 1). **Live parity verified exact** on ACSS/AIML/AP CSP (212/314/439 observations, 0 diff after normalization — `scripts/parity-mastery-batch.js`) and end-to-end (re-sync wrote 212 ACSS scores, 0 with a bad/`undefined` assignment id). **This collapses the per-topic loop — the mastery analog of the #55 bulk submissions endpoint.** |
